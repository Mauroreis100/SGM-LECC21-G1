...
//ESTA IMPLEMENTAÇÃO COLOCA TODOS OS DADOS DO VECTOR NUMA LISTA MULTIDIMENSIONAL PARA A TABELA
		...
		for (int i = 0; i < temp.size(); i++) {
			for (int j = 0; j < 5; j++) {
				...
				System.out.println(((Cliente) temp.get(j)).toString());
			}
		}
	...
	public void actionPerformed(ActionEvent e) {
		if ((e.getSource() == bt_Criar)) {
			if (!(jt_nome.getText().equals("") && jt_BI.getText().equals("")&& jt_cell.getText().equals(""))) {
				...
				if (crudCliente.existe(jt_nome.getText(), temp)) {
					JOptionPane.showMessageDialog(null,"O Cliente com BI " + jt_BI.getText() + " já existe","ATENÇÃO", JOptionPane.WARNING_MESSAGE); // OK
				} else {
					cliente = new Cliente(Integer.parseInt(jt_id.getText()), jt_nome.getText(),jt_BI.getText(),jt_cell.getText(), Double.parseDouble(jt_saldo.getText())); // Preenchendo
									// objecto
									// com o
									// formulário
					temp.add(cliente);// Actualização do vector temporário com o novo objecto
					if (crudCliente.gravarClientes(temp)) {//
						JOptionPane.showMessageDialog(null, "Cliente Registrado com sucesso", "SUCESSO",JOptionPane.PLAIN_MESSAGE);// MENSAGEM DE SUCESSO
		...
		if (e.getSource() == bt_Eliminar) {
			...
			Cliente cli = crudCliente.produtoStock(codigo, temp);
			if (cli != null) {
				int opcao = JOptionPane.showConfirmDialog(null,"Confirma que pretende apagar o cliente"+cli.getNome()+" do sistema?", "ELIMINAR",JOptionPane.YES_NO_OPTION);// YES apaga mesmo e NO não faz nada,
				if (opcao == 0) {
					crudCliente.gravarClientes(crudCliente.removerCliente(temp, codigo));// GRAVA UMA REMOÇÃO FEITA
					JOptionPane.showMessageDialog(null, "Eliminado com sucesso!", "ELIMINADO", JOptionPane.ERROR_MESSAGE); // OK
					FecharListarCliente();
					new ListaClientes();
				}
			} else {
